...
then
  ...
  curl -b session=$ADVENT_OF_CODE_SESSION https://adventofcode.com/${YEAR}/day/${DAY_WITHOUT_ZERO}/input | head -c -1 > ./inputs/$YEAR/$DAY/input.txt || echo "Failed to download input from Advent of Code!"
fi
